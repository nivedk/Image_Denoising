...
parser.add_argument('--trainlogcsv', type=str, default='trainlog.csv', help='path to the log file')
parser.add_argument('--testlogcsv', type=str, default='testlog.csv', help='path to the log file')
...
train_log_writer = csv.writer(open(opt.trainlogcsv, 'w'))
train_log_writer.writerow(['epoch', 'iteration', 'd_loss', 'g_loss'])
...
for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        train_log_writer.writerow([epoch, iteration, loss_d.item(), loss_g.item()])
    ...
    with torch.no_grad():
        for batch in testing_data_loader:
            ...
            ssim = pytorch_ssim.ssim(prediction, target)
            avg_psnr += psnr
            avg_ssim += ssim
